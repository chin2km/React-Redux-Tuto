        ...
        if (this.props.fetcher.fetched === true) {
            return (
                ...
                <List>
                    ...
                    {this.props.fetcher.starships.map(starship =>
                        <ListItem className="list-item"
                            onClick={() => this.props.fetchDataById(starship)}
                            key={starship.name}
                            primaryText={starship.name}
                            leftAvatar={<div><StarWarAvatar gender={starship.gender} size="small" /></div>}
...
const mapStateToProps = (state, ownProps) => {
    return {
        fetcher: state.StarshipsReducers.landing
    }
}
