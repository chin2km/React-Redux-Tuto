        ...
        if (this.props.fetcher.fetched === true) {
            return (
                ...
                <List>
                    ...
                    {this.props.fetcher.planets.map(planet =>
                        <ListItem className="list-item"
                            onClick={() => this.props.fetchDataById(planet)}
                            key={planet.name}
                            primaryText={planet.name}
                            leftAvatar={<div><StarWarAvatar gender={planet.gender} size="small" /></div>}
...
const mapStateToProps = (state, ownProps) => {
    return {
        fetcher: state.PlanetsReducers.landing
    }
}
