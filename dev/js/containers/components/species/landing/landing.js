        ...
        if (this.props.fetcher.fetched === true) {
            return (
                ...
                <List>
                    ...
                    {this.props.fetcher.species.map(species =>
                        <ListItem className="list-item"
                            onClick={() => this.props.fetchDataById(species)}
                            key={species.name}
                            primaryText={species.name}
                            leftAvatar={<div><StarWarAvatar gender={species.gender} size="small" /></div>}
...
const mapStateToProps = (state, ownProps) => {
    return {
        fetcher: state.SpeciesReducers.landing
    }
}
