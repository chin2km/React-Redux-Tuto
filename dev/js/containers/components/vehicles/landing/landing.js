        ...
        if (this.props.fetcher.fetched === true) {
            return (
                ...
                <List>
                    ...
                    {this.props.fetcher.vehicles.map(vehicle =>
                        <ListItem className="list-item"
                            onClick={() => this.props.fetchDataById(vehicle)}
                            key={vehicle.name}
                            primaryText={vehicle.name}
                            leftAvatar={<div><StarWarAvatar gender={vehicle.gender} size="small" /></div>}
...
const mapStateToProps = (state, ownProps) => {
    return {
        fetcher: state.VehiclesReducers.landing
    }
}
